...
import { betCandidates, categories, CategoryId, claimCount, compositeFor, evidenceByCategory, kalshiMarketSource, marketAudit, markets, marketSource, sourceMeta, sportsbookMarketAudit, teams, winAggregates, type Evidence, type Team } from './data';
...
function Markets() {
  ...
  return <section className="reportShell">
    <SectionIntro kicker="Market expectations" title="Sportsbooks and Kalshi, reconciled." copy="Sportsbook probabilities come from same-book paired Over/Under prices and proportional de-vigging. Kalshi supplies all 17 team-win tails, allowing monotone midpoint expected wins, conference/division totals and executable-side disagreement scans." />
    ...
    <div className="divisionGrid">{Object.entries(groups).map(([group,clubs])=>{const total=winAggregates.divisions.find(item=>item.label===group);return <article className="divisionCard" key={group} tabIndex={0} aria-label={`${group} win market table; scroll horizontally for all columns`}><header><span>{group.split(' ')[0]}</span><h3>{group.split(' ')[1]}</h3><small>{total ? `${wins(total.midpoint_estimate)} modeled wins` : ''}</small></header><div className="marketTable"><div className="marketHead"><span>Team</span><span>SB line</span><span>Paired O / U</span><span>SB no-vig Over</span><span>SB 50% bound</span><span>Kalshi E[W]</span><span>Bid–ask E[W]</span><span>K rank</span><span>Coverage</span><span>Action avg.</span></div>{clubs!.map((t:Team)=>{const market=markets[t.abbr];return <div className="marketRow" key={t.abbr}><span><b>{t.abbr}</b>{t.name}<small>Sportsbook ordinal rank #{market.sportsbookMarketRank}</small></span><strong>{market.line}</strong><span><b>{market.over} / {market.under}</b><small>{market.book}</small></span><span><b>{percent(market.noVigOver)}</b><small>{percent(market.hold)} hold · {market.pairedQuoteCount} quotes</small></span><span>{market.medianBracket}</span><span><b>{market.expectedWins?.toFixed(2)}</b><small>midpoint model</small></span><span>{market.expectedWinsBid.toFixed(1)}–{market.expectedWinsAsk.toFixed(1)}</span><span><b>#{market.marketRank}</b></span><span><b>{market.coverageLabel}</b><small>{market.thresholdCount} / 17 · {market.confidence} spread confidence</small></span><em>{compositeFor(t.abbr).toFixed(2)}</em></div>})}</div></article>})}</div>
    <div className="disclaimer"><strong>What is—and is not—estimated</strong><p>Kalshi E[W] is the sum of all 17 spread-weighted, monotone tail midpoints; it is modeled from observed bids and asks, not directly quoted by Kalshi. The aggregate range sums marginal bid/ask curves and is intentionally wide where distant strikes are illiquid. Scanner edges compare snapshots taken about 40 minutes apart, before Kalshi fees or slippage, and can disappear immediately.</p></div>
  </section>;
}
