...
import { betCandidates, CategoryId, claimCount, defaultAnalysisWeights, defaultProfileRanks, evidenceByCategory, kalshiMarketSource, marketAudit, markets, marketSource, previewEvidenceByTeam, previewSources, profileRanksFor, profileScoreFor, scoredCategories as categories, sourceMeta, sourceRegistry, sportsbookMarketAudit, teams, weightingModel, winAggregates, type Evidence, type Market, type PreviewEvidence, type PreviewSource, type Team } from './data';
...
function Briefing({go}:{go:(tab:TabId)=>void}) {
  ...
  return <section className="reportShell">
    <SectionIntro kicker="Executive briefing" title="Six scored lenses. Four scoped previews." copy="This is a faithful evidence digest, not a new power ranking. The dependence-aware profile fixes a 55-point offensive-family budget, assigns 30 points to defense and 15 to cross-unit coaching. The NFC East, NFC North and two-part AFC previews add team context at zero weight because they do not supply a comparable league-wide contract." />
...
function Sources() {
  return <section className="reportShell">
    ...
    <div className="versionNote"><strong>Edition 6 · Aug. 26, 2026</strong><span>{categories.length} complete unit/organizational rankings plus {previewSources.length} zero-weight team previews, dependence-aware adjustable priors, paired de-vigged sportsbook prices and complete Kalshi win-tail ladders. Offense and defense are now incorporated; special teams and an independent schedule model remain pending. Market snapshots remain append-only.</span></div>
  </section>;
}
