...
import { betCandidates, categories, CategoryId, claimCount, defaultAnalysisWeights, defaultProfileRanks, evidenceByCategory, kalshiMarketSource, marketAudit, markets, marketSource, profileRanksFor, profileScoreFor, sourceMeta, sportsbookMarketAudit, teams, winAggregates, type Evidence, type Market, type Team } from './data';

type TabId = string;
const tabs: {id:TabId; label:string}[] = [
  {id:'briefing',label:'Briefing'}, {id:'matrix',label:'League matrix'}, {id:'teams',label:'Team profiles'},
  ...categories.map(category=>({id:category.id,label:category.label})),
  {id:'markets',label:'Win markets'}, {id:'analysis',label:'Analysis vs market'}, {id:'synthesis',label:'Synthesis'}, {id:'sources',label:'Sources & QA'},
];
...
function Briefing({go}:{go:(tab:TabId)=>void}) {
  ...
  return <section className="reportShell">
    <SectionIntro kicker="Executive briefing" title="Four lenses. One incomplete but revealing map." copy="This is a faithful evidence digest, not a new power ranking. The provisional profile gives QB the most influence, then coaching, line and skill; defense and future episodes will change both the score and its weights." />
...
function Sources() {
  return <section className="reportShell">
    <SectionIntro kicker="Provenance & completeness" title="Built so omissions and interpretation can be audited." copy={`${categories.length} source transcripts were acquired read-only, snapshotted privately and hashed before extraction. The public artifact retains publisher links and checksums without exposing private-library identifiers or raw copyrighted text.`} />
    <div className="qaBanner"><strong>PASS</strong><div><h3>{categories.length*teams.length} expected ranks · {categories.length*teams.length} represented · 0 duplicates · 0 missing teams</h3><p>Every registered category contains each NFL team exactly once, in a complete 1–32 order.</p></div></div>
    ...
    <div className="versionNote"><strong>Edition 3 · Aug. 23, 2026</strong><span>{categories.length} current unit/organizational episodes, provisional importance weights, paired de-vigged sportsbook prices and complete Kalshi win-tail ladders. Defense is pending; the category registry is ready. Market snapshots remain append-only.</span></div>
  </section>;
}
