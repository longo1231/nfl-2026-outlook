...
function Briefing({go}:{go:(tab:TabId)=>void}) {
  ...
  return <section className="reportShell">
    <SectionIntro kicker="Executive briefing" title="Four scored lenses. Two scoped previews." copy="This is a faithful evidence digest, not a new power ranking. The provisional profile gives QB the most influence, then coaching, line and skill. The new NFC East and AFC Part 1 previews add team context at zero weight until a comparable league-wide contract exists." />
    ...
    <div className="readMe"><strong>How to use this</strong><p>Begin in the matrix for league shape, open a team profile for every supporting point, and use Team previews for the 12 newly covered clubs. Treat market disagreements as research prompts—not automatic bets.</p></div>
  </section>;
}
  ...
  return <section className="reportShell">
    ...
    <div className="previewSourceList">{previewSources.map(source=>{
      ...
      return <article className="previewSourceCard" key={source.id}>
        ...
        <div className="ballotGrid"><div><div className="eyebrow">Exact source ballots</div>{source.ballots.map(ballot=><div className="ballot" key={`${source.id}-${ballot.speaker}`}><strong>{ballot.speaker}</strong><small>{ballot.scope}{ballot.complete?'':' · incomplete'}</small><ol>{ballot.positions.map(position=><li key={`${position.rank}-${position.team}`}><b>{position.rank}</b><button onClick={()=>openTeam(position.team)}>{position.team}</button><span>{teamByAbbr[position.team].name}</span></li>)}</ol>{ballot.note&&<p>{ballot.note}</p>}</div>)}</div><div><div className="eyebrow">Kalshi scoped order</div>{groups.map(([group,clubs])=><div className="ballot marketBallot" key={group}><strong>{group}</strong><small>current modeled E[W] order</small><ol>{[...clubs!].sort((a,b)=>markets[a.abbr].expectedWins-markets[b.abbr].expectedWins).reverse().map((team,index)=><li key={team.abbr}><b>{index+1}</b><button onClick={()=>openTeam(team.abbr)}>{team.abbr}</button><span>{markets[team.abbr].expectedWins.toFixed(2)} modeled wins</span></li>)}</ol></div>)}</div></div>
...
function PreviewKalshiComparison({threshold,openTeam}:{threshold:number;openTeam:(abbr:string)=>void}) {
  ...
  return <div className="previewCompare"><header><div><span>Separate module · scoped source ballots</span><h3>Preview ballots versus Kalshi ≥{threshold}-win tails</h3></div><p>Division-only ordinal comparison. It does not create a league score, probability forecast or betting signal.</p></header><div className="previewCompareTable"><div className="previewCompareHead"><span>Source</span><span>Speaker</span><span>Exact or stated order</span><span>Kalshi scoped tail order</span><span>Stated-slot gap</span></div>{rows.map(({source,ballot,tailOrder,statedGap})=><div className="previewCompareRow" key={`${source.id}-${ballot.speaker}`}><span><b>{source.short}</b><small>{ballot.scope}</small></span><strong>{ballot.speaker}</strong><span className="orderLine">{ballot.positions.map(position=><button key={position.team} onClick={()=>openTeam(position.team)}>{position.rank}. {position.team}</button>)}</span><span className="orderLine">{tailOrder.map((abbr,index)=><button key={abbr} onClick={()=>openTeam(abbr)}>{index+1}. {abbr} <small>{percent(probabilityAtLeast(markets[abbr].distribution,threshold))}</small></button>)}</span><span><b>{statedGap}</b><small>sum of absolute rank gaps{ballot.complete?'':' · stated slots only'}</small></span></div>)}</div><footer><span>AFC West has no source ballot and is intentionally absent here.</span><span>Preview analysis is market-aware and remains weight 0.</span></footer></div>;
}
...
function Sources() {
  return <section className="reportShell">
    ...
    <div className="qaBanner"><strong>PASS</strong><div><h3>{categories.length*teams.length} expected scored ranks · {categories.length*teams.length} represented · 0 duplicates · 0 missing teams</h3><p>All four scoring-eligible categories contain each NFL team exactly once. Two partial, market-aware previews cover 12 teams and are explicitly weight 0.</p></div></div>
    ...
    <div className="versionNote"><strong>Edition 4 · Aug. 24, 2026</strong><span>{categories.length} complete unit/organizational rankings plus {previewSources.length} zero-weight team previews, provisional importance weights, paired de-vigged sportsbook prices and complete Kalshi win-tail ladders. Defense and AFC Part 2 are pending. Market snapshots remain append-only.</span></div>
  </section>;
}
