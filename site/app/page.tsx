...
import { betCandidates, CategoryId, claimCount, decisionSystemAudit, defaultAnalysisWeights, defaultProfileRanks, evidenceByCategory, kalshiMarketSource, marketAudit, markets, marketSource, previewEvidenceByTeam, previewSources, profileRanksFor, profileScoreFor, scoredCategories as categories, sourceMeta, sourceRegistry, sportsbookMarketAudit, teams, weightingModel, winAggregates, type Evidence, type Market, type PreviewEvidence, type PreviewSource, type Team } from './data';

type TabId = string;
const tabs: {id:TabId; label:string}[] = [
  {id:'briefing',label:'Briefing'}, {id:'system',label:'Decision system'}, {id:'matrix',label:'League matrix'}, {id:'teams',label:'Team profiles'},
...
function DecisionSystem() {
  ...
  return <section className="reportShell">
    <SectionIntro kicker="Meta review · source-of-truth boundary" title="Reliable research infrastructure. Not yet a complete betting system." copy="The corpus, source contracts and market math are auditable. A calibrated forecast, claim-level freshness, private decision records and outcome feedback are still missing, so the dashboard should not imply a fair probability or an executable recommendation it does not yet possess." />
    <div className="systemVerdict">
      <article className="ready"><span>Evidence</span><strong>Ready</strong><p>{quality.editorial_sources} unique sources, {quality.scoring_cells_represented}/{quality.scoring_cells_expected} scored cells and qualitative preview coverage for all {quality.preview_unique_teams} teams.</p></article>
      <article className="guarded"><span>Markets</span><strong>Guarded</strong><p>Paired prices and complete 17-tail curves are strong research inputs, but candidate differences remain pre-fee and lack minimum-size, quote-age and persistence gates.</p></article>
      <article className="missing"><span>Forecast</span><strong>Missing</strong><p>The weighted profile is an ordinal research index. It is not an outcome-calibrated win distribution, fair probability or learned model.</p></article>
      <article className="missing"><span>Decision & portfolio</span><strong>Missing · private</strong><p>No thesis ledger, target price, invalidation rule, stake/risk view, position history, CLV or outcome review exists yet.</p></article>
    </div>
    <div className="systemBoundary"><strong>What “truth” means here today</strong><p>Reproducible provenance and explicit as-of snapshots—not certainty. Source facts, market facts and Field Guide synthesis remain separate. Before a wager, the missing forecast and private decision layers must be supplied rather than inferred from a rank gap.</p></div>
    <div className="systemLayerGrid">
      <article><span>1 · Evidence ledger</span><h3>What informed analysts said</h3><ul><li>Immutable private snapshots and public checksums</li><li>Exact ranking and ballot contracts</li><li>Dependence and market-awareness flags</li><li className="gap">Next: atomic preview claims, people, locators and review dates</li></ul></article>
      <article><span>2 · Forecast layer</span><h3>What we believe now</h3><ul><li className="gap">Separate probability model with uncertainty</li><li className="gap">Preseason prior plus explicit weekly updates</li><li className="gap">Historical holdout and calibration tests</li><li>Keep the current weighted profile as a diagnostic input</li></ul></article>
      <article><span>3 · Market layer</span><h3>What can be traded</h3><ul><li>Append-only paired sportsbook and Kalshi snapshots</li><li>{quality.kalshi_complete_team_ladders} complete team ladders and all group totals</li><li>{quality.capture_mismatch_seconds.toFixed(3)}s paired-capture mismatch disclosed</li><li className="gap">Next: fees, minimum size, quote age, movement and persistence</li></ul></article>
      <article><span>4 · Private decision layer</span><h3>What to do—and why</h3><ul><li className="gap">Thesis, fair-price range, target and limit</li><li className="gap">Catalyst, invalidation, confidence and risk cap</li><li className="gap">Positions, correlated exposure, CLV and postmortem</li><li>Must remain outside the public artifact</li></ul></article>
    </div>
    <div className="systemPriorities">
      <header><span>Recommended build order</span><h3>Close decision risk before adding more scoring sophistication.</h3></header>
      <ol><li><b>P0</b><div><strong>Private decision ledger</strong><p>Make every contemplated wager answerable by thesis, acceptable price, invalidation and portfolio risk.</p></div></li><li><b>P0</b><div><strong>Freshness and current-state manifest</strong><p>Give injuries, claims, forecasts and quotes their own effective time, review due and stale-after rules.</p></div></li><li><b>P1</b><div><strong>Calibrated forecast layer</strong><p>Build probabilities separately from the ordinal profile; expose uncertainty and validate out of sample.</p></div></li><li><b>P1</b><div><strong>Execution-aware market board</strong><p>Add fees, minimum size, best executable price, quote age and cross-snapshot persistence.</p></div></li><li><b>P2</b><div><strong>Weekly learning loop</strong><p>Freeze preseason priors, version weekly updates, and score forecasts and decisions against closing markets and outcomes.</p></div></li></ol>
    </div>
    <div className="readMe"><strong>Edition 7 meta-audit result</strong><p>{quality.preview_claim_fragments} preview evidence fragments are retained, but zero preview team records yet carry atomic claim IDs, source locators or a people index. That is the clearest evidence-lineage debt. The full diagnosis and phased target architecture are preserved in the repository meta review and timestamped readiness audit.</p></div>
  </section>;
}
...
function Markets({openTeam}:{openTeam:(abbr:string)=>void}) {
  ...
  return <section className="reportShell">
    ...
    <div className="disclaimer"><strong>What is—and is not—estimated</strong><p>Each profile’s exact-win density is the difference between adjacent monotone Kalshi tail midpoints. Kalshi E[W] is their probability-weighted mean, not a directly quoted expected-win contract. The aggregate range sums marginal bid/ask curves and is not a jointly executable portfolio guarantee. The current marginal midpoint sum is {winAggregates.league.midpoint_estimate.toFixed(3)} wins, {Math.abs(winAggregates.league.midpoint_minus_maximum_team_wins).toFixed(3)} {winAggregates.league.midpoint_minus_maximum_team_wins>=0?'above':'below'} the 272-game ceiling; that incoherence is disclosed rather than normalized away.</p></div>
  </section>;
}

function CrossMarketScanner() {
  return <div className="edgeBoard analysisScanner"><header><div><span>Separate module · market versus market</span><h3>Cross-market scanner</h3></div><p>Same-threshold, paired sportsbook probabilities versus executable Kalshi asks. This module does not use the podcast profile.</p></header><div className="scannerMeta"><p><strong>Snapshots:</strong> sportsbooks {marketSource.retrieved}; Kalshi {kalshiMarketSource.retrieved} · {decisionSystemAudit.quality_profile.capture_mismatch_seconds.toFixed(3)}s apart.</p><p>Minimum 5¢ pre-fee edge · maximum 12¢ spread · any positive displayed top-of-book size.</p></div><div className="edgeTable"><div className="edgeHead"><span>Team</span><span>Contract</span><span>Sportsbook p</span><span>Kalshi ask</span><span>Pre-fee edge</span><span>Spread</span><span>Top size</span></div>{betCandidates.map(candidate=><a className="edgeRow" href={`https://external-api.kalshi.com/trade-api/v2/markets/${candidate.kalshi_ticker}`} target="_blank" rel="noreferrer" key={`${candidate.kalshi_ticker}-${candidate.side}`}><b>{candidate.team}</b><span>{candidate.contract}</span><span>{percent(candidate.sportsbook_probability)}</span><span>{percent(candidate.kalshi_ask)}</span><strong>+{candidate.pre_fee_edge_cents.toFixed(2)}¢</strong><span>{(candidate.kalshi_spread*100).toFixed(0)}¢</span><span>{candidate.available_size?.toLocaleString()??'—'}</span></a>)}</div><p className="scannerFootnote">Timestamped research candidates only. Prices are not simultaneous; edge excludes fees and slippage. The current filter has no minimum notional or persistence requirement, so thin rows belong on a watchlist, not an action list.</p></div>;
}
...
function MarketAnalysis({openTeam}:{openTeam:(abbr:string)=>void}) {
  ...
  return <section className="reportShell">
    ...
    <div className="analysisGuardrail"><strong>Interpretation guardrail</strong><p>Profile rank is a weighted ordering of podcast inputs, not a forecasted win distribution. Defense is represented, and schedule enters only through the composite offense episode; special teams, an independent schedule model and changing injuries remain missing. A positive tail gap means only that the profile ranks better than Kalshi’s league ordering at that threshold.</p></div>
...
function Synthesis({openTeam}:{openTeam:(abbr:string)=>void}) {
  ...
  return <section className="reportShell">
    <SectionIntro kicker="Cross-category synthesis" title="Where the inputs reinforce—and contradict—one another." copy="This layer keeps source opinion separate from derived analysis. It uses six exact Action category ranks and Kalshi's complete-ladder modeled expected-win order; special teams, an independent schedule model and changing injuries remain missing." />
...
function Sources() {
  return <section className="reportShell">
    ...
    <div className="versionNote"><strong>Edition 7 · Aug. 27, 2026</strong><span>{categories.length} complete unit/organizational rankings plus {previewSources.length} zero-weight team previews covering all 32 teams, dependence-aware adjustable priors, paired de-vigged sportsbook prices and complete Kalshi win-tail ladders. The new decision-system audit distinguishes the trusted research layers from the missing forecast and private decision layers. Market snapshots remain append-only.</span></div>
  </section>;
}
...
export default function Home() {
  ...
  return <main>
    <header className="masthead"><div className="eyebrow">2026 NFL Outlook Field Guide</div><div className="mastheadRow"><div><h1>The shape of every team</h1><p className="deck">Auditable football evidence, complete market curves and an explicit path toward a true decision system.</p></div><div className="edition"><span>Preseason · edition 7</span><strong>Data through Aug. 27</strong></div></div></header>
